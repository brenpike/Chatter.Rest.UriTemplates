...
/// caller-supplied <see cref="ISet{T}"/> of associative-array pairs. It is a snapshot,
...
/// changing the expanded URI. Backing the snapshot with a list keeps every entry that
/// was observed, in the order it was observed, which is what the expander used to see
/// when it enumerated the caller's set directly.
/// </para>
/// <para>
...
/// caller-defined order and must be canonicalized. A snapshot arriving as a plain list
/// would take the expander's ordered-sequence branch instead, making the expansion
/// depend on the caller's set implementation — the very thing the set branch exists to
/// prevent. Preserving the interface is the point; the backing store is free to differ.
/// </para>
/// <para>
...
/// mutators because the snapshot is immutable once taken, and the membership and
/// set-relation predicates because answering them would require the very relation this
/// type refuses to invent.
/// </para>
/// </summary>
internal sealed class UriTemplateSetSnapshot : ISet<KeyValuePair<string, string>>
{
    private readonly List<KeyValuePair<string, string>> _entries = new List<KeyValuePair<string, string>>();

    /// <summary>
    /// Records one observed entry. Unlike <see cref="ISet{T}.Add"/> this never rejects a
    /// duplicate, because the source set already decided what a duplicate is.
    /// </summary>
    ...
    private static NotSupportedException Unsupported() =>
        new NotSupportedException(
            $"{nameof(UriTemplateSetSnapshot)} is an immutable snapshot of the entries observed in a " +
