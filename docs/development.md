...
- **Test data:** inline for every test class except `UriTemplateComplianceTests`, which reads JSON fixtures from `TestData/`. The core test csproj copies `uritemplate-test/*.json` (four files from the submodule) into `TestData/` at build time; the compliance tests read all four — `spec-examples.json`, `extended-tests.json`, `negative-tests.json`, `spec-examples-by-section.json`. A guard fact, `AllCopiedTestDataFiles_AreExercisedByATheory`, asserts every copied `TestData/*.json` fixture is exercised by a theory
