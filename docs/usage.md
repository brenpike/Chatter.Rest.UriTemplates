...
- `IEnumerable<KeyValuePair<string, string>>` that is not an `IDictionary` (e.g., `List<KeyValuePair<string, string>>`) — associative array value, expanded in the supplied enumeration order, verbatim. An empty sequence is treated as undefined. See [Associative-array pair order](#associative-array-pair-order).
...
|---|---|
| `IDictionary<string, string>` (e.g. `Dictionary<string, string>`), including `UriTemplateValue.From(IDictionary<string, string>)` | Keys sorted by `string.CompareOrdinal` |
| An `IEnumerable<KeyValuePair<string, string>>` that is **not** an `IDictionary` (e.g. `List<KeyValuePair<string, string>>`) | The supplied enumeration order, preserved verbatim, duplicate keys included |

A `Dictionary<string, string>` has no order the caller can control or reason about — two logically identical dictionaries can enumerate differently depending on insertion and removal history — so its keys are sorted to make the expansion reproducible. **Callers who need a specific pair order should pass a `List<KeyValuePair<string, string>>`.**
