...
- **Exceptions raised by caller-supplied enumeration are outside it.** If a caller's own `IEnumerable<string>` or `IEnumerable<KeyValuePair<string, string>>` throws while expansion is reading it — from `MoveNext()`, from a `yield return` body, or from anything else the caller's code runs during enumeration — expansion propagates that exception unchanged. Its message is whatever the caller's code produced, so it can carry supplied values, including secrets. The library deliberately does not sanitize or wrap such an exception: discarding the caller's own diagnostic detail would cost more than it protects. A caller that logs expansion failures and treats its values as sensitive must account for the exceptions its own enumeration raises; only the messages above are covered.
