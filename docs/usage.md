...
- **Per-entry memoization.** Within a single `Expand` call, a caller-supplied sequence is read at most once per entry: the first expression that names the entry drains the sequence, records its members, and replays the recording for every later expression naming the same variable. The unit of memoization is the entry -- the binding of one variable name to one value -- not the value object. A single sequence instance bound to several referenced names is read once per name, and each name after the first observes whatever a fresh enumeration of the already-drained sequence happens to yield: nothing, a throw, or different members. That outcome is determined by the sequence, not by the library -- a sequence-dependent consequence, not a promise. Memoization is scoped to the call: nothing is replayed across `Expand` calls.
