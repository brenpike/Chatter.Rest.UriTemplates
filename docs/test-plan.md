...
|---|---|---|---|
| Literal expansion | Sections 2.1 and 3.1 require literals outside expressions to be valid URI-template literals and to percent-encode non-URI characters as UTF-8. | Literal text is validated and encoded by `UriTemplateParser.ProcessLiteral`. Every ASCII character outside the Section 2.1 `literals` production is rejected with `FormatException` (space, C0 controls, DEL, lone `{`/`}`, `"`, `'`, `<`, `>`, `\`, `^`, `` ` ``, `\|`), a bare `%` is rejected unless it starts a valid triplet, non-ASCII is UTF-8 percent-encoded, and unpaired surrogates are rejected. | None. |
...
| Docs accuracy | `docs/architecture.md` and `docs/usage.md` describe literal text as unchanged and list `%` as reserved passthrough. | The implementation no longer copies literal text unchanged (see the Literal expansion row); the prose in those two documents has not yet been resynced, and the `%` passthrough claim is tracked by the Reserved expansion `%` handling row. | Resync the literal-text wording in `docs/architecture.md` and `docs/usage.md`. |
