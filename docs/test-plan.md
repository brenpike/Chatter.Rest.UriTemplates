...
| Docs accuracy | `docs/architecture.md` and `docs/usage.md` describe literal text as unchanged and list `%` as reserved passthrough. | The implementation no longer copies literal text unchanged (see the Literal expansion row); the prose in those two documents has not yet been resynced, and the `%` passthrough claim is tracked by the Reserved expansion `%` handling row. The resync must also cover the non-ASCII scalar rule: only `ucschar` and `iprivate` scalars are percent-encoded, everything else is rejected. | Resync the literal-text wording in `docs/architecture.md` and `docs/usage.md`. |
